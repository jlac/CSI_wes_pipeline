...
##
for i in BAM VCF BATCH_QC HLA inbreeding CNV_100 gVCFs
do
    if ! test -d $i; then
...
if ! test -e "batches"; then
  mkdir -p gVCFs
  perl /data/GRIS_NCBR/resources/software/split_gvcf_batches.pl masterkey.txt 50 hg19
fi
...
if [ "$2" == "npr" ]
then
    snakemake -npr --snakefile CSI_wes_pipeline/scripts/hgsc_wgs_synthetic_exome_hg19.snakemake
fi

if [ "$2" == "process" ]
then
    snakemake --stats snakemake.stats --rerun-incomplete -j 150 --cluster "$CLUSTER_OPTS" --cluster-config CSI_wes_pipeline/resources/processing_cluster.json --keep-going --snakefile CSI_wes_pipeline/scripts/hgsc_wgs_synthetic_exome_hg19.snakemake 2>&1|tee -a batch_processing.log
fi