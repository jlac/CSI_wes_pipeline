...
if [ "$2" == "npr" ]
then
    snakemake -npr --snakefile CSI_wes_pipeline/scripts/hgsc_batch_processing_wgs_hg38.snakemake
fi

if [ "$2" == "process" ]
then
    snakemake --stats snakemake.stats --restart-times 1 --rerun-incomplete -j 150 --cluster "$CLUSTER_OPTS" --cluster-config CSI_wes_pipeline/resources/processing_cluster_locus.json --keep-going --snakefile CSI_wes_pipeline/scripts/hgsc_batch_processing_wgs_hg38.snakemake 2>&1|tee -a csi_batch_processing.log
fi